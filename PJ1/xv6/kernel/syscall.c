...
fetchint(struct proc *p, uint addr, int *ip)
{
  ...
  offset = p->parent == NULL ? 0 : START_ADDRESS;
  if(addr < offset || addr >= p->sz || addr + 4 > p->sz)
    return -1;
  *ip = *(int*)(addr);
  return 0;
}
...
argptr(int n, char **pp, int size)
{
  ...
  offset = proc->parent == NULL ? 0 : START_ADDRESS;
  if(argint(n, &i) < 0)
    return -1;
  if((uint)i < offset || (uint)i >= proc->sz || (uint)i+size > proc->sz)
    return -1;
  *pp = (char*)i;
  return 0;
}
